<template>
    <div class="skills-container w-full  bg-black gap-20 relative overflow-hidden py-52 ">
<!-- <span class="text-lightpink transform -rotate-90 font-bold absolute top-[750px]v -left-[700px] border-2 border-lightred p-4">CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE · CURRICULUM VITAE</span> -->

<div class="skills-top-container flex justify-center items-center gap-40 w-full h-full md:flex-col ">
  <div class="top-left-container-relative relative  w-[520px] md:w-[88%]">
    <div class="backpicture w-[100%] h-[100%]  border-2 border-lightpink absolute top-4 right-4 "></div>

  <div class="skills-top-left-container bg-lightred h-fit p-12 border-2 border-darkred relative">

    <div class="title-experiences mb-10">
  <span class="text-[5rem] relative font-bold text-darkred md:text-[3rem]">
    Expériences
    <span class="text-lightpink font-relation text-[3rem] absolute left-4 top-7 md:text-[2rem] md:top-3 ">
      Professionnelles
    </span>
  </span>
</div>
<div class="content-experiences ">
  <ul class="flex flex-col gap-10 text-darkred text-lg">
    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Sep 2024 - Sep 2025 </span>
      <span class=""><span class="">Alternance</span> <span class="font-bold text-[1.5rem]">- TOMY HOUSE</span></span>
      <span class="text-[1.1rem]">Chargée de communication</span>
    </div>
    </li>

    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Jui 2024 - Aou 2024 </span>
      <span class=""><span class="">Stage</span> <span class="font-bold text-[1.5rem]">- TOMY HOUSE</span></span>
      <span class="text-[1.1rem]">Chargée de communication</span>
    </div>
    </li>
      
    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Jan 2024</span>
      <span class=""><span class="">Stage</span> <span class="font-bold text-[1.5rem]">- BIENVOIR OPTICIEN</span></span>
      <span class="text-[1.1rem]">Community Manager</span>
    </div>
    </li>

    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Sep 2022 - Jan 2023 </span>
      <span class=""><span class="">Alternance</span> <span class="font-bold text-[1.5rem]">- OH MY CASE</span></span>
      <span class="text-[1.1rem]">Community Manager</span>
    </div>
    </li>

  </ul>
</div>
</div>
</div>




<div class="top-right-container-relative relative w-[520px] md:w-[90%]">
  <div class="backpicture w-[100%] h-[100%]  border-2 border-lightpink absolute top-4 right-4 "></div>

  <div class="skills-top-right-container bg-lightred h-fit p-12 border-2 border-darkred relative">

    <div class="title-formations mb-10">
  <span class="text-[5rem] relative font-bold text-darkred md:text-[3rem]">
    Formations
    <span class="text-lightpink font-relation text-[3rem] absolute left-5 top-7 md:text-[2rem] ">
      Scolaires
    </span>
  </span>
</div>
<div class="content-experiences ">
  <ul class="flex flex-col gap-10 text-darkred text-lg">
    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Sep 2025 - Aou 2026 </span>
      <span class=""><span class="">Bachelor</span> <span class="font-bold text-[1.5rem]">- CREATION NUMERIQUE</span></span>
      <span class="text-[1.1rem]">MyDigitalSchool</span>
    </div>
    </li>

    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Sep 2023 - Aou 2025 </span>
      <span class=""><span class="">BTS</span> <span class="font-bold text-[1.5rem]">- COMMUNICATION</span></span>
      <span class="text-[1.1rem]">Aftec</span>
    </div>
    </li>
      
    <li class="flex items-center gap-5">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#F9B29C" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Sep 2020 - Jui 2022 </span>
      <span class=""><span class="">Lycée</span> <span class="font-bold text-[1.5rem]">- STMG</span></span>
      <span class="text-[1.1rem]">Arcisse de Caumont</span>
    </div>
    </li>

    <li class="flex items-center gap-5 text-lightred">
      <div class="left-list-container ">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16"><path fill="#DE3A31" fill-rule="evenodd" d="M6 1a1.75 1.75 0 0 0-1.75 1.75V4H3a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-1.25V2.75A1.75 1.75 0 0 0 10 1zm4.25 3V2.75A.25.25 0 0 0 10 2.5H6a.25.25 0 0 0-.25.25V4zM3 5.5h10a.5.5 0 0 1 .5.5v1h-11V6a.5.5 0 0 1 .5-.5m-.5 3V13a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5V8.5H9V10H7V8.5z" clip-rule="evenodd"/></svg>
    </div>
    <div class="right-list-container flex flex-col justify-center text-d">
      <span class="date text-[1.1rem]">Jui 2024 - Aou 2024 </span>
      <span class=""><span class="">Stage</span> <span class="font-bold text-[1.5rem]">- TOMY HOUSE</span></span>
      <span class="text-[1.1rem]">Chargée de communication</span>
    </div>
    </li>

  </ul>
</div>
</div>
</div>


</div>



<div class="skills-bottom-container bg-black w-full pt-52">
  <div class="title-experiences mb-10 w-full ml-48 md:ml-0 md:text-center">
  <span class="text-[6rem] relative font-bold text-darkred">
    Skills
    <span class="text-lightpink font-relation text-[4rem] absolute left-6 top-7 ">
      Logiciels
    </span>
  </span>
</div>
  <!-- Section avec défilement horizontal -->
  <section class="horizontal-scroll-section  bg-black relative overflow-hidden ">
          <div class="flex w-full justify-center items-center flex-wrap">
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M3 12c0-4.243 0-6.364 1.318-7.682S7.758 3 12 3s6.364 0 7.682 1.318S21 7.758 21 12s0 6.364-1.318 7.682S16.242 21 12 21s-6.364 0-7.682-1.318S3 16.242 3 12"/><path d="M6.79 15.79V12m0 0V9.276c0-.11 0-.165.004-.211c.044-.45.4-.806.85-.85c.046-.004.101-.004.211-.004h1.303a1.895 1.895 0 1 1 0 3.789zm10.388-.737c-.164-.659-.935-1.158-1.862-1.158c-1.047 0-1.895.637-1.895 1.421c0 .785.848 1.421 1.895 1.421c1.046 0 1.895.637 1.895 1.421c0 .785-.849 1.421-1.895 1.421c-.93 0-1.704-.502-1.864-1.165"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M3 12c0-4.243 0-6.364 1.318-7.682S7.758 3 12 3s6.364 0 7.682 1.318S21 7.758 21 12s0 6.364-1.318 7.682S16.242 21 12 21s-6.364 0-7.682-1.318S3 16.242 3 12"/><path d="m12.947 15.79l-.82-2.653m-4.864 2.652l.82-2.652m0 0l.687-2.218c.558-1.806.838-2.708 1.335-2.708c.498 0 .777.902 1.336 2.708l.686 2.218m-4.043 0h4.043m3.662 2.652v-4.736m0-2.369v-.473"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M3 12c0-4.243 0-6.364 1.318-7.682S7.758 3 12 3s6.364 0 7.682 1.318S21 7.758 21 12s0 6.364-1.318 7.682S16.242 21 12 21s-6.364 0-7.682-1.318S3 16.242 3 12"/><path d="M14.842 11.053v3.79c0 1.044-.49.946-1.42.946a2.368 2.368 0 0 1 0-4.736zm0 0V8.21m-6.631.001v7.578"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" color="#DE3A31"><path d="M10.8 12L6 14.5c0 1.178 0 1.268.352 1.634c.351.366.917.366 2.048.366h4.801c1.132 0 1.698 0 2.05-.366s.351-.456.351-1.634M10.8 12L18 8.25M10.8 12L6 9.5c0-1.179 0-1.268.352-1.634C6.703 7.5 7.269 7.5 8.4 7.5h4.801c1.132 0 1.698 0 2.05.366s.351.455.351 1.634M10.8 12l7.2 3.75"/><path d="M2.498 12c0-4.478 0-6.718 1.391-8.109S7.519 2.5 11.998 2.5c4.478 0 6.717 0 8.109 1.391c1.39 1.391 1.39 3.63 1.39 8.109c0 4.478 0 6.718-1.39 8.109c-1.392 1.391-3.63 1.391-8.11 1.391c-4.478 0-6.717 0-8.108-1.391S2.498 16.479 2.498 12"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><path fill="#DE3A31" d="M14.782 3.153c-.231.02-.472.04-.703.07a8.5 8.5 0 0 0-2.832.834a9 9 0 0 0-2.46 1.777c-.03.04-.09.06-.141.05a7.4 7.4 0 0 0-1.496-.07a7.4 7.4 0 0 0-2.932.763c-1.768.884-3.013 2.26-3.736 4.108a7.1 7.1 0 0 0-.462 2.139c0 .05-.01.09-.02.13v.773c.02.201.05.392.07.593c.1.813.332 1.596.703 2.33c.824 1.646 2.089 2.851 3.786 3.594a7.1 7.1 0 0 0 2.45.593q.048 0 .086.01h8.576c.183-.017.362-.035.547-.06a8.3 8.3 0 0 0 2.811-.834a8.84 8.84 0 0 0 3.646-3.304a8.2 8.2 0 0 0 1.184-3.093c.05-.34.08-.692.121-1.034c0-.05.01-.09.02-.13v-.794c-.02-.23-.05-.452-.05-.662a8.4 8.4 0 0 0-.834-2.812a8.95 8.95 0 0 0-3.324-3.645a8.2 8.2 0 0 0-3.072-1.175c-.362-.06-.713-.09-1.075-.13c-.05 0-.09-.01-.14-.02zm.369 1.693c2.126.005 3.93.826 5.395 2.455a6.93 6.93 0 0 1 1.616 3.323c.15.764.181 1.547.07 2.32c-.19 1.346-.702 2.55-1.576 3.605a7.08 7.08 0 0 1-3.997 2.45a7.3 7.3 0 0 1-2.56.1c-1.095-.14-2.099-.501-3.003-1.154a5 5 0 0 1-.672-.573c-1.226-1.205-2.44-2.42-3.666-3.625c-.301-.3-.321-.632-.18-.934a.82.82 0 0 1 .863-.472c.21.02.372.141.522.292c1.105 1.114 2.2 2.209 3.304 3.324a5.26 5.26 0 0 0 3.093 1.536c1.948.261 3.605-.341 4.92-1.798c.713-.793 1.145-1.747 1.326-2.811c.26-1.587-.11-3.013-1.095-4.268c-.873-1.115-2.018-1.808-3.404-2.059c-1.416-.25-2.751.02-3.966.794c-.03.02-.1.03-.131.01a9 9 0 0 0-1.406-.854s-.01-.01-.02-.03a6.6 6.6 0 0 1 1.255-.823a6.65 6.65 0 0 1 2.641-.784a9 9 0 0 1 .67-.024zM7.546 7.509c1.455-.024 2.791.525 3.982 1.63c.854.802 1.637 1.636 2.46 2.47c.231.23.281.522.171.833s-.362.462-.683.512a.72.72 0 0 1-.632-.23c-.784-.784-1.567-1.557-2.34-2.35c-.633-.653-1.386-1.025-2.27-1.186c-1.958-.351-3.936.784-4.639 2.641c-.904 2.36.522 5.031 2.982 5.594c.482.11.995.11 1.497.1c.14-.01.22.04.32.13c.483.473.995.945 1.497 1.416c.03.03.07.06.1.09c-.06 0-.1.01-.14.01h-2.3a5.83 5.83 0 0 1-5.693-4.568c-.653-2.942 1.034-5.925 3.926-6.798a6.3 6.3 0 0 1 1.762-.294"/></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><path fill="#DE3A31" d="M23.63 15.3c-.71-.745-2.166-1.17-4.224-1.17c-1.1 0-2.377.106-3.761.354a19.4 19.4 0 0 1-2.307-2.661c-.532-.71-.994-1.49-1.42-2.236c.817-2.484 1.207-4.507 1.207-5.962c0-1.632-.603-3.336-2.342-3.336c-.532 0-1.065.32-1.349.781c-.78 1.384-.425 4.4.923 7.381a60 60 0 0 1-1.703 4.507c-.568 1.349-1.207 2.733-1.917 4.01C2.834 18.53.314 20.34.03 21.758c-.106.533.071 1.03.462 1.42c.142.107.639.533 1.49.533c2.59 0 5.323-4.188 6.707-6.707c1.065-.355 2.13-.71 3.194-.994a35 35 0 0 1 3.407-.745c2.732 2.448 5.145 2.839 6.352 2.839c1.49 0 2.023-.604 2.2-1.1c.32-.64.106-1.349-.213-1.704zm-1.42 1.03c-.107.532-.64.887-1.384.887c-.213 0-.39-.036-.604-.071c-1.348-.32-2.626-.994-3.903-2.059a18 18 0 0 1 2.98-.248c.746 0 1.385.035 1.81.142c.497.106 1.278.426 1.1 1.348zm-7.524-1.668a38 38 0 0 0-2.945.674a40 40 0 0 0-2.52.745a40 40 0 0 0 1.207-2.555c.426-.994.78-2.023 1.136-2.981c.354.603.745 1.207 1.135 1.739a50 50 0 0 0 1.987 2.378M10.038 1.46a.77.77 0 0 1 .674-.425c.745 0 .887.851.887 1.526c0 1.135-.355 2.874-.958 4.861c-1.03-2.768-1.1-5.074-.603-5.962M6.134 17.997c-1.81 2.981-3.549 4.826-4.613 4.826a.87.87 0 0 1-.532-.177c-.213-.213-.32-.461-.249-.745c.213-1.065 2.271-2.555 5.394-3.904"/></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"><path d="M21 7v10a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4"/><path d="M7 8v8h4m3-5.5V13m0 3v-3m0 0s0-2.5 3-2.5"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><g fill="none" stroke="#DE3A31" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" color="#DE3A31"><path d="M2.5 12c0-4.478 0-6.718 1.391-8.109S7.521 2.5 12 2.5c4.478 0 6.718 0 8.109 1.391S21.5 7.521 21.5 12c0 4.478 0 6.718-1.391 8.109S16.479 21.5 12 21.5c-4.478 0-6.718 0-8.109-1.391S2.5 16.479 2.5 12"/><path d="M6.75 11.985v-2.98c0-.282.232-.51.518-.51h1.978c.44 0 .892.075 1.22.365c.35.31.69.79.674 1.43c-.02.75-.566 1.633-1.367 1.695c-.8.062-1.552 0-3.022 0m0 0v2.983c0 .282.232.51.518.513c1.14.01 1.804.051 2.505-.003c.8-.061 1.347-.946 1.367-1.694c.016-.641-.324-1.121-.674-1.431c-.328-.29-.78-.366-1.22-.366zm10.045 2.98c-.438.425-2.572 1.264-3.341-.944c-.08-.221-.144-.697-.107-1.064m0 0c.064-.626.34-1.503 1.364-1.861c1.468-.513 2.391.91 2.63 1.861zm1.242-3.956h1.36"/></g></svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><path fill="#DE3A31" d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12s12-5.373 12-12S18.627 0 12 0M6.962 7.68c.754 0 1.337.549 1.405 1.2c.069.583-.171 1.097-.822 1.406c-.343.171-.48.172-.549.069c-.034-.069 0-.137.069-.206c.617-.514.617-.926.548-1.508c-.034-.378-.308-.618-.583-.618c-1.2 0-2.914 2.674-2.674 4.629c.103.754.549 1.646 1.509 1.646c.308 0 .65-.103.96-.24c.5-.264.799-.47 1.097-.8c-.073-.885.704-2.046 1.851-2.046c.515 0 .926.205.96.583c.068.514-.377.582-.514.582s-.378-.034-.378-.17c-.034-.138.309-.07.275-.378c-.035-.206-.24-.274-.446-.274c-.72 0-1.131.994-1.029 1.611c.035.275.172.549.447.549c.205 0 .514-.31.617-.755c.068-.308.343-.514.583-.514c.102 0 .17.034.205.171v.138c-.034.137-.137.548-.102.651c0 .069.034.171.17.171c.092 0 .436-.18.777-.459c.117-.59.253-1.298.253-1.357c.034-.24.137-.48.617-.48c.103 0 .171.034.205.171v.138l-.136.617c.445-.583 1.097-.994 1.508-.994c.172 0 .309.102.309.274c0 .103 0 .274-.069.446c-.137.377-.309.96-.412 1.474c0 .137.035.274.207.274s.685-.206 1.096-.754l.007-.004c-.002-.068-.007-.134-.007-.202c0-.411.035-.754.104-.994c.068-.274.411-.514.617-.514c.103 0 .205.069.205.171c0 .035 0 .103-.034.137c-.137.446-.24.857-.24 1.269c0 .24.034.582.102.788c0 .034.035.069.07.069c.068 0 .548-.445.89-1.028c-.308-.206-.48-.549-.48-.96c0-.72.446-1.097.858-1.097c.343 0 .617.24.617.72c0 .308-.103.65-.274.96h.102a.77.77 0 0 0 .584-.24a.3.3 0 0 1 .134-.117c.335-.425.83-.74 1.41-.74c.48 0 .924.205.959.582c.068.515-.378.618-.515.618l-.002-.002c-.138 0-.377-.035-.377-.172s.309-.068.274-.376c-.034-.206-.24-.275-.446-.275c-.686 0-1.13.891-1.028 1.611c.034.275.171.583.445.583c.206 0 .515-.308.652-.754c.068-.274.343-.514.583-.514c.103 0 .17.034.205.171c0 .069 0 .206-.137.652c-.17.308-.171.48-.137.617c.034.274.171.48.309.583c.034.034.068.102.068.102c0 .069-.034.138-.137.138c-.034 0-.068 0-.103-.035c-.514-.205-.72-.548-.789-.891c-.205.24-.445.377-.72.377c-.445 0-.89-.411-.96-.926a1.6 1.6 0 0 1 .075-.649c-.203.13-.422.203-.623.203h-.17c-.447.652-.927 1.098-1.27 1.303a.9.9 0 0 1-.377.104c-.068 0-.171-.035-.205-.104c-.095-.152-.156-.392-.193-.667c-.481.527-1.145.805-1.453.805c-.343 0-.548-.206-.582-.55v-.376c.102-.754.377-1.2.377-1.337a.074.074 0 0 0-.069-.07c-.24 0-1.028.824-1.166 1.373l-.103.445c-.068.309-.377.515-.582.515c-.103 0-.172-.035-.206-.172v-.137l.046-.233c-.435.31-.87.508-1.075.508c-.308 0-.48-.172-.514-.412c-.206.274-.445.412-.754.412c-.352 0-.696-.24-.862-.593c-.244.275-.523.553-.852.764c-.48.309-1.028.549-1.68.549c-.582 0-1.097-.309-1.371-.583c-.412-.377-.651-.96-.686-1.509c-.205-1.68.823-3.84 2.4-4.8c.378-.205.755-.343 1.132-.343m9.77 3.291c-.104 0-.172.172-.172.343c0 .274.137.583.309.755a1.7 1.7 0 0 0 .102-.583c0-.343-.137-.515-.24-.515z"/></svg>
        </div>
  </section>

</div>
</div>
  </template>
  
  <script>
  export default {
    name: "Ciriculum",

  };
  </script>
  
  
  
  <style scoped>
    #intro {
    -webkit-text-fill-color: transparent;
    -webkit-text-stroke-width: 1.6px;
  }
  </style>
  